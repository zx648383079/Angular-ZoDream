...
export class DraggableDirective {
    ...
    // 发射增量数据
    @Output()
    public launchMouseIncrement = new Subject<IPoint>();
    ...
    public getHostElement(): Element {
        return document.querySelector(this.appTarget);
    }
    ...
    public listenMouseDownMove(event: MouseEvent): void {
        ...
        this.listenMove$ = fromEvent(this.getHostElement(), 'mousemove').subscribe((move: MouseEvent) => {
            this.zone.run(() => {
                ...
                this.launchMouseIncrement.next({ y: move.movementY, x: move.movementX });
            });
        });
