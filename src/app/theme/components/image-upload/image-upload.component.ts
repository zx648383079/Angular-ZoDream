...
import { Configs } from '../../constants/theme.constants';
...
export class ImageUploadComponent implements OnInit, OnChanges {
  ...
  @Input() url: string = Configs.host + 'upload';
  ...
  ngOnInit() {
    ...
    ? this.supportedExtensions.map((ext) => 'image/' + ext) : ['image/*'];
  }
  ...
  private uploadSingleFile(fileHolder: FileHolder, url = this.url, customForm?: { [name: string]: any }) {
    if (url) {
      ...
      this.imageService
        .upload(this.url, fileHolder.file, this.headers, this.partName, customForm, this.withCredentials)
        .subscribe(
